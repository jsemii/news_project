...
## 25. 산업/직무와 무관한 사회면 뉴스가 importance_score를 높게 받아 브리핑 상위에 노출됨
#### 현상
"8세 소녀, 美 호수 수영 후 '뇌 먹는 아메바' 감염… 투병 끝 사망" 같은 일반 사회면
기사가 `importance_score=8`을 받아 브리핑 상위(취준생의 직무/산업 브리핑)에
노출됐다. `news_industry`에는 산업 태그가 하나도 없었는데도(`industries: []`)
높은 점수를 받았다.

#### 원인
1차 규칙 기반 필터(`NewsRelevanceFilter`)는 제목 키워드 배제(코스피/환율/부고 등),
수집 경과일, 본문 길이만 확인할 뿐 사건·사고·사회면 뉴스를 걸러내는 로직이 없다.
근본 원인은 AI 구조화 프롬프트(`OpenAiClient.buildGeneralSystemPrompt`)의
`importance_score` 기준 문구였다 — "취준생의 산업 이해나 직무 역량과 크게 관련
없는 단순 사실 전달이면 낮게(1~3점)"처럼 "관련 없으면 낮게"라는 소극적 표현만
있고, "직무/산업과 무관해도 사회적으로 화제성 있는 사건은 무조건 낮은 점수"라는
강제 규칙이 없었다. 그래서 LLM이 이 뉴스를 "생명과 관련된 충격적 사건 = 사회적
중요도가 높은 뉴스"로 해석해 8점을 준 것으로 보인다. 즉 "뉴스 자체의 사회적
중요도"와 "이 프로젝트가 타겟하는 3개 직무(IT전산/데이터분석/백엔드)와의
관련성"을 프롬프트가 명확히 분리하지 못한 것이 원인이었다.

#### 해결
`importance_score` 기준 문단을 4단계 구간(1~2/3/4~6/7~10점)으로 재작성하고,
각 구간을 "3개 직무 또는 산업 목록과의 직접적 연관성"으로만 정의했다. 특히
1~2점 구간에 "사건·사고, 재해, 연예, 스포츠, 날씨, 부고/인사 등은 아무리
사회적으로 중요하거나 충격적이어도 무조건 이 구간"이라고 명시해서, 사회적
중요도와 직무 관련성을 혼동하지 않도록 강제했다. 자체 점검 목록에도 "사회적
중요도가 아니라 직무/산업 관련성만 기준으로 삼았는가"를 묻는 항목을 추가했다.
`news_industry` 태그 유무로 코드에서 필터링하는 방식은 쓰지 않았다 — 태그가
없어도 관련 있는 뉴스가 있을 수 있고, 있어도 무관한 뉴스가 있을 수 있어서
프롬프트의 판단 기준 자체를 고치는 게 맞다고 판단했다.

실제 두 기사로 전/후 비교 검증했다(로컬에서 임시 JUnit 테스트로 두 기사를
직접 크롤링해 `analyzeGeneral`을 재호출, 확인 후 테스트 파일은 삭제):
...
## 27. 오늘 뉴스 브리핑이 안 보이는 문제가 재발함(18번 항목의 근본 원인 미해결)
#### 현상
운영(`http://3.37.221.184/`)에서 "오늘의 브리핑"이 빈 배열로 나왔다. 18번
항목("오래된 백로그가 먼저 처리되어 최신 뉴스가 밀림")과 겉보기엔 같은
증상인데, 그때 19번 항목에서 TOO_OLD 자동 제외 기능까지 추가해서 "해결"했다고
생각했던 문제가 며칠 뒤 다시 발생했다.

#### 원인
19번 항목의 수정(수집된 지 2일 넘은 뉴스를 자동 제외)은 "아주 오래된 뉴스가
무한정 쌓이는" 문제만 막았을 뿐, `NewsAnalysisMapper.selectUnanalyzedNews`의
FIFO 정렬(`collected_at` 오름차순)과 처리 용량 자체는 그대로였다. 즉 2일
이내의 뉴스는 여전히 전부 처리 대상이고, 오늘 새로 들어온 뉴스는 항상 그 줄의
맨 뒤에 선다. 그런데 처리 용량은 스케줄러 기준 하루 3회(08/13/18시) x
20건 = 하루 60건으로 고정돼 있었다. 실제로 EC2 DB에서
`SELECT count(*) FROM news WHERE collected_at >= now() - interval '7 days'`로
확인한 최근 7일 평균 수집량은 **하루 120건**이었다 — 처리 용량의 딱 2배다.
즉 매일 약 60건씩 백로그가 순증하는 구조였고, 18번 항목 때 수동으로 드레인해서
0으로 만든 것도 원인을 없앤 게 아니라 증상만 지운 것이라 며칠 뒤 다시 쌓여
같은 문제가 재발했다.

#### 해결
`application.yml`의 `openai.schedule.cron`을 하루 3회(`0 0 8,13,18 * * *`)에서
매시간(`0 0 * * * *`)으로 바꿨다 — 처리 용량이 하루 480건(24회 x 20건)이 되어
실측 수집량(120건/일) 대비 4배 여유가 생긴다. 배치가 실행돼도 밀린 게 없으면
`selectUnanalyzedNews`가 0건을 돌려주고 바로 끝나므로(처리한 만큼만 OpenAI를
호출), 비용은 늘지 않고 지연만 해소된다. 이번에도 운영 EC2의 실제 백로그를
드레인(139건 → 0건)해서 오늘 브리핑이 다시 정상적으로 뜨는 것까지 확인했다.
앞으로 참고할 점: "밀린 백로그를 수동으로 비웠다"는 증상 완화일 뿐 근본 해결이
아니다 — 재발하지 않으려면 처리 용량 자체가 실제 수집량을 웃도는지 수치로
확인해야 한다.

## 28. MyBatis XML에 부등호(<=)를 그대로 썼다가 SAXParseException / UNION ORDER BY 에러
#### 현상
직무 탭에 1순위/2순위 로직을 추가하면서 `BriefingMapper.xml`에 `row_number()
... rn <= #{limit}` 같은 SQL을 그대로 적었더니, 서버 기동 자체가
`SAXParseException: The content of elements must consist of well-formed
character data or markup`로 실패했다. `&lt;`로 고치고 나니 이번엔 다른
에러(`PSQLException: invalid UNION/INTERSECT/EXCEPT ORDER BY clause`
— "Only result column names can be used, not expressions or functions")가
나서 직무 탭 조회(`GET /api/briefings?job=...`)가 500을 냈다.

#### 원인
1. MyBatis XML은 결국 XML이라, SQL 안에 `<=`나 `<`처럼 부등호를 그대로 쓰면
   파서가 태그 시작(`<`)으로 잘못 해석한다. XML 주석(`<!-- -->`) 안에서는
   허용되지만(주석은 마크업으로 파싱되지 않음), 실제 `<select>` 본문 안에서는
   반드시 `&lt;`로 escape하거나 `<![CDATA[ ... ]]>`로 감싸야 한다.
2. PostgreSQL은 `UNION ALL` 바로 뒤에 오는 `ORDER BY`에는 결과 컬럼명만 쓸 수
   있고, `CASE WHEN ... END`처럼 표현식은 허용하지 않는다(1순위/2순위를 합친
   뒤 "1순위면 직무 점수, 2순위면 공통 점수로 정렬"하려고 CASE를 썼다가 걸림).

#### 해결
1. `rn <= #{limit}`처럼 실제 SQL 본문에 들어가는 부등호는 전부 `rn &lt;=
   #{limit}`로 바꿨다.
2. `UNION ALL`로 합친 결과를 `combined`라는 CTE로 한 번 더 감싸고, 그 바깥의
   일반 `SELECT * FROM combined ORDER BY ...`에서 `CASE` 표현식으로 정렬했다
   (PostgreSQL 에러 메시지가 직접 알려준 해결책 — "move the UNION into a FROM
   clause"를 그대로 따름). 두 문제 모두 실제 서버 기동 + API 호출로 재현하고
   고친 뒤 다시 확인했다.

## 29. 직무별 importance_score 프롬프트가 "관련 없으면 1~2점" 규칙을 안 지킴
#### 현상
직무별 중요도 점수를 추가한 뒤 실제 배치로 검증했더니, 사회면 기사(예: "5년째
물 밖으로 못 나오는 인도 소년" 투병기)의 공통 점수(`news_analysis.importance_score`,
같은 뉴스)는 정확히 1점이었는데, 같은 뉴스의 직무별 점수는 IT전산 4점/
데이터분석 6점/백엔드 5점으로 훨씬 높게 나왔다. 프롬프트에 "무관하면 무조건
1~2점", "사회적 파장 배제" 규칙을 이미 넣었는데도 지켜지지 않았다.

#### 원인
`buildJobsSystemPrompt()`에 있던 기존 지시("직접적인 관련이 적어 보여도 그
직무 관점에서 최대한 의미를 찾아 작성하세요 — why_it_matters/key_skills가
비어있지 않게 하려는 지시")가, 원래는 텍스트 필드에만 적용하려던 것이었는데
같은 응답 안에서 함께 매기는 importance_score에도 "새어 들어간" 것으로 보인다.
LLM이 "무관해도 의미를 짜내서 문장을 쓰다 보니, 그 그럴듯한 문장에 맞춰 점수도
덩달아 후하게 준" 것으로 추정된다.

#### 해결
두 단계로 강화했다.
1. "이 지시는 why_it_matters/key_skills 텍스트에만 해당하고 importance_score에는
   적용하지 않는다"는 문장과 "텍스트를 그럴듯하게 채웠다고 점수를 덩달아
   높이면 안 된다"는 경고를 명시적으로 추가했다 — 그런데 이것만으로는
   부분적으로만 개선됐다(4~6점 → 3~4점, 여전히 1~2점 규칙 미준수).
2. **결정적 수정**: JSON 응답 스키마와 지시 순서 자체를 바꿔서, 각 직무마다
   `importance_score`를 **먼저** 결정하고 `why_it_matters`/`key_skills`를
   **그 다음에** 쓰도록 했다("점수를 먼저 정하고, 그 점수에 맞춰 텍스트를
   써라" — 반대로 텍스트를 먼저 고민하면 점수가 텍스트에 맞춰 끌려 올라가는
   문제가 있었기 때문). 이 순서 변경 이후 같은 무관한 기사가 3개 직무 전부
   정확히 1점으로 나오는 것을 실제 OpenAI 호출로 확인했다. 대조군으로 백엔드와
   명확히 관련된 가상 기사(분산 DB 도입, 백엔드 200명 채용)를 같이 테스트해서,
   백엔드만 9점이고 나머지 두 직무는 4~5점으로 낮게 나오는 것도 확인했다(독립
   판단 원칙도 함께 검증됨). 이후 임시 검증용 테스트 파일은 삭제했다.

앞으로 참고할 점: 한 프롬프트 안에 "텍스트를 풍부하게 채워라"는 지시와 "점수는
엄격하게 매겨라"는 지시가 같이 있으면, 모델이 텍스트 관대함을 점수에도 전이시킬

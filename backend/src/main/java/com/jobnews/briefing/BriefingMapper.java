...
public interface BriefingMapper {
    ...
    //              볼지와 몇 건까지 가져올지(limit)를 받습니다.
    // [무엇을 하고] selectTopBriefings와 같은 조건(오늘 분석된 것, importance_score
    ...
    // @Param: 파라미터가 2개 이상이면 MyBatis가 XML의 #{job}/#{limit}을 실제 어느
    // 인자와 연결할지 이름만으로는 알 수 없어서, @Param으로 이름을 명시적으로 붙여줘야 합니다.
    List<BriefingRow> selectTopBriefingsByJob(@Param("job") String job, @Param("limit") int limit);
}

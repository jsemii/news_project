...
## 15. EC2에서 backend 컨테이너가 OOM으로 강제 종료됨(exit 137)
#### 현상
EC2에 배포한 뒤 한동안(약 30분) 잘 돌던 backend 컨테이너가 아무 에러 로그도 없이
갑자기 죽었다. `docker compose ps`에는 아예 안 보였고(기본 옵션은 실행 중인 것만
표시), `docker compose ps -a`로 보니 `Exited (137)` 상태였다.

#### 원인
`free -h`로 확인해보니 EC2 인스턴스 전체 메모리가 908Mi(약 1GB, 프리티어급
인스턴스)였고 **스왑이 0B**였다. Exit code 137(= 128+9, SIGKILL)은 리눅스 OOM
killer가 메모리가 부족할 때 프로세스를 강제로 죽일 때 나는 전형적인 신호다. `backend/
Dockerfile`이 JVM 힙 크기를 전혀 제한하지 않고 있었는데(`java -jar app.jar`), JVM은
기본적으로 "보이는 메모리의 25%"까지 힙을 쓰려고 한다 — Postgres/nginx와 메모리를
나눠 쓰는 1GB 인스턴스에서는 이게 여유 메모리를 다 잡아먹기 쉽고, 스왑도 없어서
메모리가 부족해지는 순간 바로 강제 종료로 이어졌다.

#### 해결
`backend/Dockerfile`의 실행 명령을 `java -jar app.jar`에서 `java -Xmx384m -jar
app.jar`로 바꿔서 힙 상한을 명시적으로 제한했다(로컬에서 `java -XX:+PrintFlagsFinal`로
`MaxHeapSize`가 정확히 384MiB로 적용되는 것을 확인함). 이건 애플리케이션 코드/이미지
차원의 조치이고, 별도로 EC2 인스턴스 자체에 스왑(디스크를 임시 메모리처럼 쓰는 공간)을
추가하는 것도 함께 권장했다 — 힙 제한만으로는 Postgres/nginx까지 포함한 전체 메모리
압박을 완전히 막을 수 없어서, 스왑이 있으면 순간적으로 메모리가 부족해도 바로
강제 종료되지 않고 디스크로 여유분을 확보할 수 있다.
